...
document.onkeyup = function (e) {
  ...
  if (game == null || game.won == true || game.remainingGuess == 0) {
    if (e.key == "Enter" && !document.getElementById("play-button").disabled)
      startGame();
    return;
  }
...
function displayGuess(guess) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (let j = 0; j < game.guesses.length; j++) {
      if (
        game.guesses[j].includes(guess.charAt(i)) &&
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (guess.charAt(i) === game.word.charAt(i)) {
      ...
      for (let j = 0; j < game.word.length; j++) {
        if (
          guess.charAt(i) === game.word.charAt(j) &&
          letterAndCount.indexOf(guess.charAt(i)) + 1 > 0
        ) {
      ...
      if (letterAndCount[letterAndCount.indexOf(guess.charAt(i)) + 1] < 0) {
        // If the letter has already been guessed then display it in white
        guessDiv.style.backgroundColor = "white";
        guessDiv.style.boxShadow = "0px 0px 3px 3px white";
      }
      ...
      if (
        !game.word.includes(guess.charAt(i)) &&
        !game.incorrectLetters.includes(guess.charAt(i))
      ) {
...
function displayScore(score) {
  ...
  if (game.remainingGuess <= 0)
    otherText = "   - You Lose! The word was: " + game.word + ".";
...
function displayIncorrectLetters() {
  ...
  for (let i = 0; i < game.incorrectLetters.length; i++) {
    ...
    if (
      !div.textContent.includes(game.incorrectLetters[i]) &&
      game.incorrectLetters[i] != undefined
    )
      newLetters += game.incorrectLetters[i] + " ";
  }
